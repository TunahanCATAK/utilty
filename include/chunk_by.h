...
#ifndef TC_CHUNK_BY_H
#define TC_CHUNK_BY_H
...
namespace tc::views{
    ...
    template<std::ranges::forward_range V,
            std::predicate<std::ranges::range_reference_t<V>, std::ranges::range_reference_t<V>> F>
    class chunk_by_view {
    private:
        V base_;
        F func_;
        ...
        struct iterator {
            ...
            iterator() = default;
            constexpr iterator(std::ranges::iterator_t<constify<V>> current, constify<chunk_by_view>* parent): current_(std::move(current)), parent_(parent) {
    ...
    chunk_by_view(R&&, F)->chunk_by_view<std::views::all_t<R>, F>;
